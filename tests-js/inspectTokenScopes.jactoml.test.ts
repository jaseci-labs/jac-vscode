...
import { tokenizeContent, TokenizeResult, TokenInfo } from './tokenizer';

const GRAMMAR_PATH = path.join(__dirname, '..', 'syntaxes', 'jactoml.tmLanguage.json');
const WASM_PATH = require.resolve('vscode-oniguruma').replace(/main\.js$/, 'onig.wasm');
const FIXTURE = path.join(__dirname, 'fixtures', 'sample.jac.toml');
